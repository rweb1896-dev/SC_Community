...
import { HelpConversation, HelpMessage, HelpNotification, ReconnectRequest } from '../core/models';
...
export class MessageDockComponent implements OnInit,OnDestroy,AfterViewChecked,OnChanges{
 ...
 active:HelpConversation[]=[];history:HelpConversation[]=[];messages:HelpMessage[]=[];incoming:ReconnectRequest[]=[];notifications:HelpNotification[]=[];selected?:HelpConversation;
 ...
 get unreadTotal(){return this.notifications.filter(n=>!n.read).length+this.incoming.length;}
 ...
 reload(background=false){if(!background)this.loading=true;this.error='';this.api.helpConversations('ACTIVE').subscribe({next:r=>{this.active=r;this.syncSelected();},error:e=>this.error=e.error?.detail||'Active help chats could not be loaded.'});this.api.helpConversations('HISTORY').subscribe({next:r=>{this.history=r;this.syncSelected();this.loading=false;},error:e=>{this.error=e.error?.detail||'Chat history could not be loaded.';this.loading=false;}});this.api.incomingReconnects().subscribe({next:r=>this.incoming=r});this.api.helpNotifications().subscribe({next:r=>this.notifications=r});}
 ...
 openNotification(n:HelpNotification){this.api.readHelpNotification(n.id).subscribe({next:()=>{n.read=true;const c=[...this.active,...this.history].find(row=>row.id===n.conversationId);if(c){this.tab=c.status==='ACTIVE'?'ACTIVE':'HISTORY';this.select(c);}else if(n.postId){this.error=n.type==='POST_CLOSED'?'This help request is no longer active.':'Open the matching post in the feed to offer help.';}}});}
